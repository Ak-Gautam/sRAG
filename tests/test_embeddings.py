...
class TestEmbeddings(unittest.TestCase):
    ...
    def setUp(self):
        ...
        tokenizer_patcher = patch("zrag.embeddings.AutoTokenizer.from_pretrained")
        model_patcher = patch("zrag.embeddings.AutoModel.from_pretrained")

        self.addCleanup(tokenizer_patcher.stop)
        self.addCleanup(model_patcher.stop)

        mock_tokenizer_from_pretrained = tokenizer_patcher.start()
        mock_model_from_pretrained = model_patcher.start()

        tokenizer_instance = MagicMock()
        encoded = MagicMock()
        encoded.to.return_value = encoded
        encoded.__getitem__.return_value = MagicMock()
        tokenizer_instance.return_value = encoded
        mock_tokenizer_from_pretrained.return_value = tokenizer_instance

        model_instance = MagicMock()
        model_instance.to.return_value = model_instance
        model_instance.eval.return_value = None
        model_output = MagicMock(last_hidden_state=MagicMock())
        model_instance.return_value = model_output
        mock_model_from_pretrained.return_value = model_instance
